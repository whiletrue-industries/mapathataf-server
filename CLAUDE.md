...
- `functions/main.py` — Firebase entry points: `api` (HTTPS, europe-west4, wraps the Flask app), `data_processing` (scheduler, daily 00:00 Israel time), `data_processing_s` (HTTP trigger for the same pipeline), `data_export` (scheduler, daily 02:00 Israel time, public CSV export). `initialize_app()` runs here before importing `api`.
...
- `functions/process_data/__init__.py` — pipeline that creates city docs from `city_names.csv` (beware trailing commas) and cluster docs from `eshkol.csv` (`slug,name,city_links` with `;`-separated slugs).
  - Loads two files daily: the all-facilities (education) file, then the Ministry of Health Tipat Halav file. `functions/tipat_halav.py` (pure Python, unit-tested; also owns `doc_id()` = `md5(_id)[:8]`, the item doc id) coerces each station into the all-facilities shape: `info._id = tipat-<station id>`, `info.facility_kind = health`, `info.facility_sub_kind = טיפת חלב`, `info.age_group` = all four groups, one `official` record with `source = moh` (`symbol`, `name`, `address`, `phone` = first of `phone_numbers`, `email`, `activity_hours` ← opening_hours, `more_details` ← notes, plus the raw status/owner/district columns). Only active stations (`status_code == 1`) are imported; no de-duplication against education. Like the education load it only adds/merges (`info` + `official` replaced, `admin`/`user`/`key` kept) — a station that later closes or leaves the file is NOT removed.
- `functions/migrate_tipat_halav_ids.py` — one-off (dry run by default, `--apply` to write; needs `GOOGLE_CLOUD_PROJECT=mapathataf` with ADC): moves 18 hand-entered Tipat Halav items onto the doc ids of their official records so the import merges into them. Hand-paired list inside. Delete once applied.
- `functions/export_data/__init__.py` — daily public CSV export of the whole database to `gs://mapathataf.firebasestorage.app/exports/facilities.csv` (public URL `https://storage.googleapis.com/mapathataf.firebasestorage.app/exports/facilities.csv`, overwritten in place). Rows are `api.process_item()` at public privilege (so unpublished / soft-deleted items and `_private_` fields never reach it), one row per item across all non-cluster workspaces. Columns mirror the admin Excel export in the frontend repo (`workspace`, `workspace_city`, `id`, then `resolved_`, `admin_`, `owner_`, `info_`, `official_<source>[N]_`), all lowercase snake_case; `resolve_item()` is a port of `resolveItem()` in the frontend's `api.service.ts` — keep the two in sync. At most 3 official records per source get columns, the rest go as JSON into `official_<source>_more_records`; inline `data:` values (owner photos) are dropped.
